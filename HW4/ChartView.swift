...
struct ChartView: View {
    ...
    init(charactersData: CharactersData){
        for index in charactersData.characters{
            ...
            if index.job == "弓手"{
                jobSum[0] += 1
            }else if index.job == "要塞"{
                jobSum[1] += 1
            }else if index.job == "修士"{
                jobSum[2] += 1
            }else if index.job == "術師"{
                jobSum[3] += 1
            }else if index.job == "士兵"{
                jobSum[4] += 1
            }else if index.job == "騎士"{
                jobSum[5] += 1
            }else if index.job == "法師"{
                jobSum[6] += 1
            }else if index.job == "刺客"{
                jobSum[7] += 1
            }
...
struct BarChart: View
{
    ...
    var body: some View
    {
        HStack(spacing: 16)
        {
            Bar(barHeights: self.barHeights[0], info: "弓手")
            Bar(barHeights: self.barHeights[1], info: "要塞")
            Bar(barHeights: self.barHeights[2], info: "修士")
            Bar(barHeights: self.barHeights[3], info: "術師")
            Bar(barHeights: self.barHeights[4], info: "士兵")
            Bar(barHeights: self.barHeights[5], info: "騎士")
            Bar(barHeights: self.barHeights[6], info: "法師")
            Bar(barHeights: self.barHeights[7], info: "刺客")
        }
        .frame(height: 270)
    }
}
